...
| **Contents** | 1 skill · 89 dysfunctions |
| **Version** | 0.27.0 |
...
It packages **89 named dysfunctions** from Trond Hjorteland's *"Organisational Dysfunction of the Day"* series, all read through the same lens: **open sociotechnical systems theory (OST)** and its DP1 (top-down bureaucracy) vs DP2 (self-managing teams) distinction.

## What's inside

One sharp, cleanly-triggering skill — `organisational-dysfunction` — built on Anthropic's progressive-disclosure pattern:

- **`SKILL.md`** — the always-loaded router. Holds the shared DP1/DP2 lens once, plus an index of all 89 dysfunctions grouped by theme.
...
### 0.27.0 — 2026-09-08
- Added `#89` **We need a resource** (88 → 89 dysfunctions). Generated via the update pipeline. Eval scenario added, but the targeted routing/triggering probe could not be executed in the unattended run (the headless `claude` CLI returns `401 OAuth access token has been revoked`, so the harness reported `trigger=False route=[]` — an auth failure, not a measurement; see the eval-harness tracking issue) — it is unverified, not passed.

### 0.26.0 — 2026-09-07
- Added `#88` **The gates they removed** (87 → 88 dysfunctions). Generated via the update pipeline. Eval scenario added, but the targeted routing/triggering probe could not be executed in the unattended run (the headless `claude` CLI returns `401 OAuth access token has been revoked` — see the eval-harness tracking issue) — it is unverified, not passed.
- 0.25.0 and 0.26.0 land in the same merge, so only `organisational-dysfunction-v0.26.0` is tagged; 0.25.0 has no tag of its own.

### 0.25.0 — 2026-09-06
- Added `#85` **The enterprise architect**, `#86` **Removing management** and `#87` **It took an outsider to say it** (84 → 87 dysfunctions). Generated via the update pipeline. Eval scenarios added for all three, but the targeted routing/triggering probe could not be executed in the unattended run (the headless `claude` CLI again hung with no output and no error until the timeout, so the harness returned empty — see the eval-harness tracking issue) — they are unverified, not passed.

### 0.24.0 — 2026-09-01
- Added `#84` **Deskilled by AI** (83 → 84 dysfunctions). Generated via the update pipeline. Eval scenario added, but the targeted routing/triggering probe could not be executed in the unattended run (the headless `claude` CLI hung with no output and no error until the timeout, so the harness returned empty) — it is unverified, not passed.
...
### 0.19.0 — 2026-08-25
- Added `#79` **The adaptation plan nobody owns** (78 → 79 dysfunctions). Generated via the update pipeline. Eval scenario added, but the targeted routing/triggering probe could not be executed in the unattended run (the headless `claude` CLI hung with no output and no error, so the harness timed out and returned empty) — it is unverified, not passed.
...
### 0.16.0 — 2026-08-20
- Added `#76` **We transformed, our suppliers didn't** (75 → 76 dysfunctions). Generated via the update pipeline; passed the targeted routing/triggering test.
