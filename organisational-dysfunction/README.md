...
| **Contents** | 1 skill · 86 dysfunctions |
| **Version** | 0.25.0 |
...
It packages **86 named dysfunctions** from Trond Hjorteland's *"Organisational Dysfunction of the Day"* series, all read through the same lens: **open sociotechnical systems theory (OST)** and its DP1 (top-down bureaucracy) vs DP2 (self-managing teams) distinction.

## What's inside

One sharp, cleanly-triggering skill — `organisational-dysfunction` — built on Anthropic's progressive-disclosure pattern:

- **`SKILL.md`** — the always-loaded router. Holds the shared DP1/DP2 lens once, plus an index of all 86 dysfunctions grouped by theme.
...
### 0.25.0 — 2026-09-03
- Added `#85` **The enterprise architect** and `#86` **Removing management** (84 → 86 dysfunctions). Generated via the update pipeline. Eval scenarios added for both, but the targeted routing/triggering probe could not be executed in the unattended run (the headless `claude` CLI again hung with no output and no error until the timeout, so the harness returned empty — see the eval-harness tracking issue) — they are unverified, not passed.
